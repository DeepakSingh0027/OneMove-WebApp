...
function Card({ index, name, category, price, link, onCardClick }) {
  return (
    <div className="w-full object-contain group hover:shadow-lg hover:scale-105 transition duration-300 ease-in-out rounded-lg">
      ...
      />
      <h4 className="text-lg font-semibold mt-2 group-hover:text-[#41290c] transition duration-300">
        {name}
      </h4>
      <p className="text-gray-600 group-hover:text-gray-800 transition duration-300">
        Category: {category}
      </p>
      <p className="text-gray-800 group-hover:text-[#775021] transition duration-300">
        &#8377; {price}
      </p>
    </div>
  );
